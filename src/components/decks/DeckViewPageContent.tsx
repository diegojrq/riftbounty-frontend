...
export interface DeckViewPageContentProps {
  deck: Deck;
  backHref: string;
  backLabel: string;
  /** Esconde botão Editar (ex.: decks pré-construídos). */
  readOnly?: boolean;
}

export function DeckViewPageContent({
  deck,
  backHref,
  backLabel,
  readOnly = false,
}: DeckViewPageContentProps) {
  const { t } = useLocale();
              ...
              {runeSorted.length === 0 ? (
                ...
                <div className="grid grid-cols-4 gap-2 sm:grid-cols-6 sm:gap-3 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12">
                  {runeSorted.map((item, i) => {
                    ...
                    return item.card ? (
                      <div key={`rune-${i}-${id}`} className="min-w-0 overflow-visible pr-1.5 pb-1.5">
                        <DeckCardThumb card={item.card} quantity={item.quantity} />
                      </div>
                    ) : (
