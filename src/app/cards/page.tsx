...
export default function CardsPage() {
  ...
  const initDomains = useMemo(() => {
    const d = searchParams?.get("domain");
    if (!d) return [];
    return d.split(",").map((s) => s.trim().toLowerCase()).filter((s) => (DOMAINS as readonly string[]).includes(s) || s === "colorless");
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  const initRarity = useMemo(() => {
    const r = searchParams?.get("rarity");
    return r && (RARITY_OPTIONS as readonly string[]).includes(r.toLowerCase()) ? r.toLowerCase() : undefined;
  }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const [nameFilter, setNameFilter] = useState<string | undefined>(initSearch.length >= MIN_SEARCH_LENGTH ? initSearch : undefined);
  const [selectedDomains, setSelectedDomains] = useState<string[]>(initDomains);
  const [selectedRarity, setSelectedRarity] = useState<string | undefined>(initRarity);
  const [selectedType, setSelectedType] = useState<string | undefined>(initType);
  const [selectedSet, setSelectedSet] = useState<string | undefined>(initSet);
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-end sm:gap-x-5 sm:gap-y-3">
        ...
        <div className="shrink-0">
          ...
          <select id="home-filter-rarity" value={selectedRarity ?? ""} onChange={(e) => setSelectedRarity(e.target.value || undefined)}
        ...
        <div className="shrink-0">
          ...
          <select id="home-filter-type" value={selectedType ?? ""} onChange={(e) => setSelectedType(e.target.value || undefined)}
            className="rounded border border-gray-600 bg-gray-800 px-2 py-1.5 text-sm text-white">
            <option value="">All</option>
    ...
    <div className="min-h-screen bg-gray-900">
      ...
      <div className="sticky top-0 z-20 border-b border-gray-700 bg-gray-900 sm:top-[77px]">
        <div className="mx-auto w-full max-w-[1600px] px-4 py-3 sm:px-6 lg:px-10 xl:px-12">
          ...
          {filtersExpanded && <div className="hidden sm:block">{filterContent}</div>}
          <div className="hidden sm:block">{activeChips}</div>
        </div>
      </div>
